...
exports.createUser = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        if (responseVerify.rowCount > 0) {
            ...
            yield database_1.pool.query('INSERT INTO users (name, email, password, social_network, active, created_at, updated_at) VALUES ($1, $2, $3, $4, $5, $6, $7)', [name, email, bcrypt.hashSync(password, 10), social_network, true, nowDate, nowDate]);
            const response = yield database_1.pool.query('SELECT * FROM users WHERE email = $1 ', [email]);
            yield database_1.pool.query('INSERT INTO role_user (role_id, user_id, created_at, updated_at) VALUES ($1, $2, $3, $4)', [rol_id, response.rows[0].id, nowDate, nowDate]);
            return res.json({
                ok: true,
                usuario: {
                    name,
...
exports.updateUser = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const nowDate = new Date();
        database_1.pool.query('UPDATE users SET name = COALESCE($1, name), identification = COALESCE($2, identification), email = COALESCE($3, email), birth_date = COALESCE($4, birth_date), updated_at = $5 WHERE id = $6', [name, identification, email, cumple, nowDate, id]);
        const response = yield database_1.pool.query('SELECT * FROM users WHERE id = $1', [id]);
        var usuarioDB = response.rows[0];
        const responseRole = yield database_1.pool.query('SELECT ro.name FROM role_user ru, roles ro WHERE ru.user_id = $1 AND ru.role_id = ro.id', [usuarioDB.id]);
        var rolesDB = responseRole.rows;
        var usuario = {
            id: usuarioDB.id,
            name: usuarioDB.name,
